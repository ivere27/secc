...
module.exports.performPumpMode = function(job, settings, cb) {
  ...
  async.waterfall([
    ...
    function(cached, files, checkFiles, fileExists, gzipPack, callback) {
      ...
      options.headers['x-secc-jobid'] = job.id;
      options.headers['x-secc-argv'] = JSON.stringify(job.remoteArgv);
      options.headers['x-secc-filename'] = path.basename(job.outputPath, path.extname(job.outputPath));
      options.headers['x-secc-outfile'] = job.outfile;
      options.headers['x-secc-driver'] = driver;
      options.headers['x-secc-cross'] = job.crossAvailable;
      options.headers['x-secc-target'] = (job.crossAvailable)
                                      ? job.compilerInformation.dumpmachine
                                      : null;

      debug(job.remoteArgv);

      debug('header[x-secc-filename] = %s', options.headers['x-secc-filename']);
