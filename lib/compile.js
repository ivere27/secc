...
function CompileStream(options) {
  ...
  this.clientOutfile = options.outfile;
    ...
    async.waterfall([
      ...
      async.each(self.outputFiles, function(outfile, cb){
        ...
        .on('error', function(err) {
          ...
          if (self.cache && self.redisClient) {
            process.nextTick(function(){
              self.redisClient.hset(redisKey, "chunkCount" , String(redisChunkCount), redisDebug);
              self.redisClient.hset(redisKey, "stdout" , self._stdout, redisDebug);
              self.redisClient.hset(redisKey, "stderr" , self._stderr, redisDebug);
              self.redisClient.hset(redisKey, "type" , (self.usingPipe) ? 'preprocessed' : 'pump', redisDebug);
