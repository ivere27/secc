...
function CompileStream(options) {
  ...
  this.compiler = '/usr/bin/' + options.compiler || '/usr/bin/gcc';
  ...
  if (this.usingPipe)  
    newArgv.push.apply(newArgv, ['-fpreprocessed', '-x', this.language,'-c','-o',this.outputPath,'-pipe','-']);
  else
    newArgv.push.apply(newArgv, ['-c','-o',this.outputPath,'-pipe']);
...
function GeneratePreprocessed(sourcePath, options) {
  ...
  debug('GeneratePreprocessed start')
  ...
  //check already preprocessedfile(ex, assembler)
...
function CompilerInformation(compilerPath, flag) {
  ...
  if (['--version','-dumpmachine','-dumpversion'].indexOf(flag) === -1)
...
var determineOutputPath = function(argv, removeItem) {
  ...
  if(argv.indexOf('-MT') !== -1) {
    outputPath = argv[argv.indexOf('-MT') + 1];
    if (removeItem) argv.splice(argv.indexOf('-MT'), 2);
  }
  if(argv.indexOf('-o') !== -1) {
    outputPath = argv[argv.indexOf('-o') + 1];
    if (removeItem) argv.splice(argv.indexOf('-o'), 2);
  }
...
var determineSourcePath = function(argv, removeItem) {
  ...
  for (var i = argv.length - 1; i >= 0; i--) {
    ...
    if (supportingExtname.indexOf(path.extname(argv[i])) !==-1 ) {
      sourcePath = argv[i];
...
var determineLanguage = function(command, compilerPath, sourcePath) {
...
var determineProjcetId = function() {
  return crypto.createHash('md5')
    .update(process.env.SECC_PROJECT || process.cwd()).digest("hex");
}
...
module.exports.determineProjcetId = determineProjcetId;
