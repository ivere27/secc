...
function Scheduler(SECC) {
  if (!(this instanceof Scheduler)) return new Scheduler(SECC);

  this.SECC = SECC || {scheduler : {}};
  this.SECC.scheduler.port = process.env.SECC_PORT || 10509;

  if (!this.SECC.archivePath || this.SECC.archivePath === '')
    this.SECC.archivePath = path.join(__dirname, '..', 'archive');
  if (!this.SECC.uploadPath || this.SECC.uploadPath === '')
    this.SECC.uploadPath = path.join(os.tmpdir(), 'secc', 'scheduler_upload');
  ...
  require('mkdirp').sync(this.SECC.archivePath);
  require('mkdirp').sync(this.SECC.uploadPath);
}
...
Scheduler.prototype.initServer = function(successCallback, failureCallback) {
  ...
  //app.use(compression);
  var SCHEDULER = {};
  SCHEDULER.am = require('./archivemanager.js')(express, io, SECC, SCHEDULER);
  SCHEDULER.cm = require('./cachemanager.js')(express, io, SECC, SCHEDULER);
  SCHEDULER.dm = require('./daemonmanager.js')(express, io, SECC, SCHEDULER);
  SCHEDULER.jm = require('./jobmanager.js')(express, io, SECC, SCHEDULER);
  SCHEDULER.om = require('./optionmanager.js')();
  ...
  var schedulerView = require('../routes/schedulerView')(express, io, SECC, SCHEDULER);
