...
var ArchiveManager = function(express, io, SECC, SCHEDULER) {
  ...
  if (!SECC.uploadPath)
    throw new TypeError('SECC uploadPath is not set');
  if (!SECC.archivePath)
    throw new TypeError('SECC archivePath is not set');

  this.archiveDatabase = path.join(this.SECC.archivePath, 'archive.json');
  this.archives = {};

  //mkdir if not exists.
  mkdirp.sync(this.SECC.uploadPath);
  mkdirp.sync(this.SECC.archivePath);
...
ArchiveManager.prototype.loadArchivesFromJson = function() {
  ...
  try {
    var data = fs.readFileSync(self.archiveDatabase);
...
ArchiveManager.prototype.saveArchivesToJSON = function() {
  ...
  try {
    var data = fs.writeFileSync(self.archiveDatabase, JSON.stringify(self.archives,null,2));
  } catch(e) {
...
ArchiveManager.prototype.addArchive = function(archive, file, cb) {
  ...
  var fs = require('fs');
  var path = require('path');
  var fromFile = path.join(self.SECC.uploadPath, file.filename);
  var toFile = path.join(self.SECC.archivePath, archive.archiveFile);
...
ArchiveManager.prototype.removeArchive = function(archiveId, cb) {
  ...
  var fs = require('fs');
  return fs.unlink(path.join(self.SECC.archivePath, self.archives[archiveId].archiveFile) , function(err){
