...
OptionManager.prototype.analyzeArguments = function(options) {
  var parsedOptions = [];
  for (var i = 0; i < options.length ; i++) {
    var o = this.parseOneOption(options[i]);
    if (o && !o.singular && options[i+1] && (options[i+1][0] !== '-'))
      o.following = options[++i];

    parsedOptions.push(o);
  }
