...
OptionManager.prototype.analyzeArguments = function(argv, driver, cwd, mode) {
  //debug('analyzeArguments start');
  var argv = argv || [];
  var driver = driver || 'gcc';
  var cwd = cwd || '';
  ...
  // if the driver is 'g++' or 'clang++' then,
  // language is c++ even '-x' is set
  if (driver === 'g++' || driver === 'clang++') {
    language = 'c++';
  } else if (formIndex.hasOwnProperty('-x')) {
  ...
  //target
  if (formIndex.hasOwnProperty('-target') || formIndex.hasOwnProperty('--target')) {
    targetSpecified = true;

    debug(formIndex['-target'])
    debug(formIndex['--target'])
    if (formIndex.hasOwnProperty('-target'))
      target = formIndex['-target'].following;
    if (formIndex.hasOwnProperty('--target'))
      target = formIndex['--target'].rvalue;
  }
  ...
  if (!useLocal  && (mode === '1')) {
    ...
    for (var i = 0; i < form.length ; i++) {
      ...
      if (!form[i].singular && form[i].following)
    ...
    for (var i = 0; i < form.length ; i++) {
      ...
      if (form[i].category === 'Diagnostic Options') continue;
      if (form[i].bare === '-I') {
  ...
  if (!useLocal  && (mode === '2')) {
    ...
    for (var i = 0; i < form.length ; i++) {
      if (form[i].bare === '-o') continue;
      if (form[i].bare === '-MF') continue;
      if (form[i].bare === '-MMD') continue;
  ...
  argvHash = crypto.createHash('md5')
            .update(JSON.stringify(remoteArgv))
            .update(String(outfile))
            .digest("hex");
