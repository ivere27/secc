...
module.exports.performPreprocessedMode = function(job, settings, cb) {
  ...
  async.waterfall([
      ...
      req.on('response', function (res) {
        ...
        res.on('end', function(){
          ...
          debug('archiveId : %s , %s ,  %s', job.archive.archiveId
            , job.archive.compilerVersion, job.archive.dumpmachine);
          ...
          if (job.cachePrefered == true && job.cacheAvailable == true) { //oh, cache~
            ...
            cacheReq.on('response', function(res) {
              ...
              if (res.headers['x-secc-stdout'] !== undefined)
                process.stdout.write(querystring.unescape(res.headers['x-secc-stdout']));
              if (res.headers['x-secc-stderr'] !== undefined)
                process.stderr.write(querystring.unescape(res.headers['x-secc-stderr']));
    ...
    function(cached, preprocessedStream, callback) {
      ...
      debug('cross : %s, remote compiler : %s, remote driver : %s, language : %s, '
            , job.crossAvailable, compiler, driver, job.language);
      debug(job.remoteArgv);

      //FIXME : this happens when new remote daemon is on.
      if (job.daemon.system === undefined)
      ...
      var options = {
        ...
        headers: {
          ...
          'x-secc-jobid' : job.id,
          'x-secc-driver' : driver,
          'x-secc-language' : job.language,
          'x-secc-argv' : JSON.stringify(job.remoteArgv),
          'x-secc-filename' : path.basename(job.outputPath, path.extname(job.outputPath)),
          'x-secc-outfile' : job.outfile,
          'x-secc-cross' : job.crossAvailable,
          'x-secc-target': (job.targetSpecified)
                          ? job.target
                          : ((job.crossAvailable)
      ...
      req.on('response', function(res) {
        if(res.statusCode !== 200 || res.headers['x-secc-code'] !== '0') {
          debug('reponse statusCode : %s, header[x-secc-code] = %s', res.statusCode, res.headers['x-secc-code']);
          this.abort();
